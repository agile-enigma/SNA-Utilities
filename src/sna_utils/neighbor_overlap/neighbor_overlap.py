...
def create_overlap_graph(
    ...
    if directed is True and incoming is True:
        graph.add_edges_from(map(lambda x: (x.target, x.source), edges_df.itertuples()))
    else:
        if directed is False and incoming is True:
            print(
                "incoming set to True even though graph is undirected. "
                + "ignoring incoming parameter..."
            )
    ...
    for node1, node2 in combinations(graph.nodes, 2):
        shared_audience = len(
            set(nx.neighbors(graph, node1)).intersection(
                set(nx.neighbors(graph, node2))
            )
        )
        if shared_audience > 0:
            overlap_graph.add_edge(
                ...
                node2,
                weight=round(
                    (shared_audience / len(list(nx.neighbors(graph, node1))) * 100), 1
                ),
            )
            overlap_graph.add_edge(
                node2,
                node1,
                weight=round(
                    (shared_audience / len(list(nx.neighbors(graph, node2))) * 100), 1
                ),
            )
    ...
    for node in overlap_graph.nodes:
        overlap_graph.nodes[node]["label"] = " ".join(
            [node + ":", str(len(list(nx.neighbors(graph, node))))]
        )
        overlap_graph.nodes[node]["value"] = len(list(nx.neighbors(graph, node)))
        overlap_graph.nodes[node]["community"] = community[node]
        overlap_graph.nodes[node]["color"] = mpl.colors.rgb2hex(
            cmap.colors[overlap_graph.nodes[node]["community"]]
        )
...
def mutual_neighbors(
    ...
    if directed is True and incoming is True:
        ...
        reversed_graph.add_edges_from(map(lambda x: (x[1], x[0]), 
                                      list(nx_graph.edges))
                                      )
        ...
        if directed is False and incoming is True:
            print(
                "incoming set to True even though graph is directed. " +
                " ignoring incoming parameter..."
            )
